...
describe("sagas", () => {
  test("watchUsers", () => {
    ...
    assert.deepStrictEqual(gen.next().value, call(dummyDappInterface.getUsers));
    assert.deepStrictEqual(gen.next(USERS).value, put(actions.setUsers(USERS)));
    assert.deepStrictEqual(gen.next().value, delay(REFETCH_CYCLE));
    assert.deepStrictEqual(gen.next().value, call(dummyDappInterface.getUsers));
    // ...
  });
});
